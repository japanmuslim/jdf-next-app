...
const QuizView: FC<QuizViewProps> = ({
  ...
  >
    {!quiz && (
      <div className="bg-white w-full lg:max-w-3xl px-8 py-10 rounded-3xl text-primary relative z-10">
    ...
    {quiz && !isFinish && (
      <div className="bg-white w-full lg:max-w-2xl px-8 py-10 rounded-3xl text-primary relative z-10 overflow-hidden">
        ...
        <div className="flex justify-between items-center my-4">
          <h1 className="text-2xl font-bold m-0">{`Question ${stepQuiz + 1}`}</h1>
          <p>
            Question: <strong>{stepQuiz}</strong>/{quizLength}
          </p>
        </div>
        <h1 className="text-2xl font-bold text-center mt-12 mb-14">
          {quiz.title}
        </h1>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {quiz.answers.map((answer) => (
            <Button
              key={answer.id}
              type="button"
              className="w-full rounded-full py-2 hover:-translate-y-1 duration-300 transition-all"
    ...
    {isFinish && (
      // <>
      //   <div className="bg-white w-full lg:max-w-2xl px-8 py-8 rounded-3xl text-primary relative z-10 shadow-lg animate-fade-in-up">
      ...
      //       <p className="text-center md:text-xl text-lg font-semibold text-gray-700">
      //         You have completed the quiz with a score of <br />
      //       </p>
      //       <h1
      //         className={`text-5xl font-bold text-center m-0 animate-scale ${
      //           score > 80
      //             ? 'text-green-500'
      //             : score > 40
      //               ? 'text-yellow-500'
      ...
      //       </h1>
      //     </div>
      //     <div className="relative md:h-80 h-40">
      //       <ReactPlayer
      //         url={
      //           score === 100
      ...
      //       <Button
      //         type="button"
      //         className="w-full rounded-full !py-4 transition duration-300 ease-in-out"
      //         size="lg"
